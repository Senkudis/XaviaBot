...
function onLoad() {
    const levelup_gifs = readdirSync(join(global.assetsPath, "levelup_gifs")).filter(file => file.endsWith(".png"));
    global.getLevelupGif = () => levelup_gifs[Math.floor(Math.random() * levelup_gifs.length)];
}

const langData = {
    "ar_SY": {
        "levelup": "{name},\n كيهيهيهي مبروك!🔥أبشر يا مكنة 😀معدل تفاعلك زاد للمستوى: ( {level} )! ",
    },
    "en_US": {
        "levelup": "{name}, Your Keyword Level 🔥 {level}!"
    }
}
